...
8. Before wrapping up, make sure every installed council member and bot sets up their scheduled tools, routines, and automations: recurring sweeps, planning sessions, check-ins, and loops, so they activate on their own and do everything their skills say they should. Offer the council's shared weekly rhythm (see `## Council Rhythm & Recurring Routines` in `skills/council/council/SKILL.md`: the Monday stand-up, the Sunday/Wednesday/Friday bookends, and the wandering gossip loop) as the default calendar. Also schedule the two Tithe Bell one-shots (one month and one year out) from that same section.
