...
- **Claude Code:** `/plugin marketplace add MindGoblinStudios/grim-tome`, then `/plugin install grim-tome@grim-tome` for the full bundle.
- **Codex CLI / ChatGPT desktop:** `codex plugin marketplace add MindGoblinStudios/grim-tome`. The catalog (grim-core, grim-council, grim-artifacts) installs by default; toggle pieces in `/plugins`.
- **Cursor:** listed in the Cursor Marketplace (submission pending). The repo carries `.cursor-plugin/marketplace.json` for it.
